const game = (() => {
    ...
    const startGame = () => {
        if (player1_name.value && player1_symbol.value && player2_name.value && player2_symbol.value) {
            let player1 = Player(player1_name.value, player1_symbol.value, 1);
            let player2 = Player(player2_name.value, player2_symbol.value, 2);
            let player = player1;
            createBoard();
        }
    }
    ...
    const createBoard = () => {
        let board_state = Array.from(Array(3), () => new Array(3));
